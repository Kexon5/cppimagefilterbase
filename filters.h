...
public:
	...
	virtual void applyFeatureFilter(image_data& img, image_data& img_copy, rectangle_area& filter_area, int i, int j) = 0;
};

class monochrome_filter: public filter {
public:
	void prepareImg(image_data& img, rectangle_area& filter_area) {};
	void applyFeatureFilter(image_data& img, image_data& img_copy, rectangle_area& filter_area, int i, int j);
};
